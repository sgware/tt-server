...
public class LogicalWorld extends WorldModel {
	...
	/**
	 * The introductory text shown to the participants when the story starts.
	 */
	private String[] introduction = new String[2];
	
	/** The describer used to describe elements of this story world */
	...
	/**
	 * Returns the introductory text that will be shown to the given role at the
	 * beginning of the story. If no value has been {@link
	 * #setIntroduction(Role, String) explicitly set}, this method returns a
	 * description of the initial state generated by {@link #describe()
	 * describing} the initial assignment of every variable.
	 * 
	 * @param role the role to whom the text will be shown
	 * @return the introductory text show to that role at the start of the story
	 */
	public String getIntroduction(Role role) {
		String text = introduction[role.ordinal()];
		if(text == null) {
			for(Variable variable : getVariables()) {
				Assignment assignment = new Assignment(variable, getInitialValue(variable));
				String description = getDescription(assignment, role);
				text += (text.isEmpty() ? "" : " ") + description;
			}
		}
		return text;
	}
	...
	/**
	 * Sets the introductory text that will be shown to the given role at the
	 * beginning of the story.
	 * 
	 * @param role the role to whom the text will be shown
	 * @param text the introductory text show to that role at the start of the
	 * story
	 */
	public void setIntroduction(Role role, String text) {
		introduction[role.ordinal()] = text;
	}
	...
	public Status start(Role role) {
		...
		initial = initial.setDescription(getIntroduction(role));
